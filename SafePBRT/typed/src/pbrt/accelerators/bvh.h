...
public:
    ...
    BVHAccel(const std::vector<Reference<Primitive> > &p, uint32_t maxPrims = 1,
             const string &sm = "sah");
    BBox<world_s> WorldBound() const;
    bool CanIntersect() const { return true; }
    ~BVHAccel();
    bool Intersect(const Ray<world_s> &ray, Intersection *isect) const;
...
private:
    ...
    BVHBuildNode *recursiveBuild(MemoryArena &buildArena,
        ...
        uint32_t *totalNodes, std::vector<Reference<Primitive> > &orderedPrims);
