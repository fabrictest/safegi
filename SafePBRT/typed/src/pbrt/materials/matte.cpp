...
BSDF *MatteMaterial::GetBSDF(const DifferentialGeometry &dgGeom,
    ...
    Spectrum<rho_d> r = Kd->Evaluate(dgs).Clamp();
...
MatteMaterial *CreateMatteMaterial(const Transform<material_s, world_s> &xform,
        const TextureParams &mp) {
    Reference<Texture<Spectrum<rho_d> > > Kd = mp.GetSpectrumTexture("Kd", Spectrum<rho_d>(mfloat<rho_d>(1.0f)));
