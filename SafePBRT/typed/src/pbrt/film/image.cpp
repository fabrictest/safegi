...
#define FILTER_TABLE_SIZE 16
    ...
    for (int y = 0; y < FILTER_TABLE_SIZE; ++y) {
            mfloat<length_d> fy = ((float)y + .5f) *
                   filter->yWidth / FILTER_TABLE_SIZE;
        for (int x = 0; x < FILTER_TABLE_SIZE; ++x) {
            mfloat<length_d> fx = ((float)x + .5f) *
...
void ImageFilm::AddSample(const CameraSample &sample,
                          const Spectrum<radiance_d> &L) {
    // Compute sample's raster extent
    mfloat<length_d> dimageX = sample.imageX - mfloat<length_d>(0.5f);
    mfloat<length_d> dimageY = sample.imageY - mfloat<length_d>(0.5f);
    ...
    for (int x = x0; x <= x1; ++x) {
        float fx = fabs((mfloat<length_d>(x) - dimageX) *
                         filter->invXWidth * (float)FILTER_TABLE_SIZE);
        ifx[x-x0] = min(Floor2Int(fx), FILTER_TABLE_SIZE-1);
    }
    int *ify = ALLOCA(int, y1 - y0 + 1);
    for (int y = y0; y <= y1; ++y) {
        float fy = fabs((mfloat<length_d>(y) - dimageY) *
                         filter->invYWidth * (float)FILTER_TABLE_SIZE);
        ify[y-y0] = min(Floor2Int(fy), FILTER_TABLE_SIZE-1);
    }
    bool syncNeeded = 
        (filter->xWidth > mfloat<length_d>(0.5f) || 
        filter->yWidth > mfloat<length_d>(0.5f));
    for (int y = y0; y <= y1; ++y) {
        for (int x = x0; x <= x1; ++x) {
...
void ImageFilm::Splat(const CameraSample &sample, const Spectrum<radiance_d> &L) {
    float xyz[3];
    L.ToXYZ(xyz);
...
void ImageFilm::GetSampleExtent(int *xstart, int *xend,
    ...
    *xstart = Floor2Int(xPixelStart + 0.5f - __asfloat(filter->xWidth));
    *xend   = Floor2Int(xPixelStart + 0.5f + xPixelCount  +
                        __asfloat(filter->xWidth));
    *ystart = Floor2Int(yPixelStart + 0.5f - __asfloat(filter->yWidth));
    *yend   = Floor2Int(yPixelStart + 0.5f + yPixelCount +
                        __asfloat(filter->yWidth));
}
