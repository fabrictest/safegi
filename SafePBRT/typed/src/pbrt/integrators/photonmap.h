...
public:
    ...
    PhotonIntegrator(int ncaus, int nindir, int nLookup, int maxspecdepth,
        int maxphotondepth, const mfloat<length_d> &maxdist, bool finalGather, int gatherSamples,
        float ga);
    ~PhotonIntegrator();
    Spectrum<radiance_d> Li(const Scene *scene, const Renderer *renderer,
        const RayDifferential<world_s> &ray, const Intersection &isect, const Sample *sample,
