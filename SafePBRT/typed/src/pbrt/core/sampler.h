...
public:
    ...
    Sampler(int xstart, int xend, int ystart, int yend,
            int spp, const mfloat<time_d> &sopen, const mfloat<time_d> &sclose);
    virtual int GetMoreSamples(Sample *sample, RNG &rng) = 0;
    virtual int MaximumSampleCount() = 0;
    virtual bool ReportResults(Sample *samples, const RayDifferential<world_s> *rays,
        const Spectrum<radiance_d> *Ls, const Intersection *isects, int count);
