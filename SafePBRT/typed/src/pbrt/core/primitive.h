...
public:
    ...
    virtual bool Intersect(const Ray<world_s> &r, Intersection *in) const = 0;
    virtual bool IntersectP(const Ray<world_s> &r) const = 0;
    ...
    virtual BSDF *GetBSDF(const DifferentialGeometry &dg,
        const Transform<object_s, world_s> &ObjectToWorld, MemoryArena &arena) const = 0;
...
public:
    ...
    virtual bool Intersect(const Ray<world_s> &r, Intersection *isect) const;
    virtual bool IntersectP(const Ray<world_s> &r) const;
    ...
    BSDF *GetBSDF(const DifferentialGeometry &dg,
                  const Transform<object_s, world_s> &ObjectToWorld, MemoryArena &arena) const;
...
public:
    ...
    TransformedPrimitive(Reference<Primitive> &prim,
                         const AnimatedTransform<world_s, world_s> &w2p)
        : primitive(prim), WorldToPrimitive(w2p) { }
    bool Intersect(const Ray<world_s> &r, Intersection *in) const;
    bool IntersectP(const Ray<world_s> &r) const;
    ...
    BBox<world_s> WorldBound() const {
        return WorldToPrimitive.InverseMotionBounds(primitive->WorldBound());
    }
private:
    // TransformedPrimitive Private Data
    Reference<Primitive> primitive;
    const AnimatedTransform<world_s, world_s> WorldToPrimitive;
};
...
public:
    ...
    const AreaLight *GetAreaLight() const;
    BSDF *GetBSDF(const DifferentialGeometry &dg,
                  const Transform<object_s, world_s> &, MemoryArena &) const;
    BSSRDF *GetBSSRDF(const DifferentialGeometry &dg,
                  const Transform<object_s, world_s> &, MemoryArena &) const;
};
