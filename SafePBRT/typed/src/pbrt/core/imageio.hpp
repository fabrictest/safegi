...
template<typename D>
RGBSpectrum<D> *ReadImageEXR(const string &name, int *width, int *height);
...
RGBSpectrum<D> *ReadImageEXR(const string &name, int *width, int *height) {
    ...
    *height = dw.max.y - dw.min.y + 1;
    ...
    RGBSpectrum<D> *ret = new RGBSpectrum<D>[*width * *height];
    for (int i = 0; i < *width * *height; ++i) {
        float frgb[3] = { rgb[3*i], rgb[3*i+1], rgb[3*i+2] };
        ret[i] = RGBSpectrum<D>::FromRGB(frgb);
    }
    delete[] rgb;
    return ret;
 ...
 */
template<typename D>
RGBSpectrum<D> *ReadImageTGA(const string &name, int *width, int *height)
{
    int                 x, y, pixbytes;
    tga_header_t        header;
    ...
    *height = imageSpec.height;
    ...
    src = srcBuf;
    RGBSpectrum<D> *ret = new RGBSpectrum<D>[*width * *height];
    RGBSpectrum<D> *dst = ret;
    for(y = *height - 1; y >= 0; y--)
        for(x = 0; x < *width; x++) {
            if (pixbytes == 1)
                *dst++ = RGBSpectrum<D>(mfloat<D>((*src++) / 255.f));
            else {
                float c[3];
                c[2] = (*src++) / 255.f;
                c[1] = (*src++) / 255.f;
                c[0] = (*src++) / 255.f;
                *dst++ = RGBSpectrum<D>::FromRGB(c);
                if (pixbytes == 4)
                    ++src;
            }
    }
