...
void
Primitive::FullyRefine(std::vector<Reference<Primitive> > &refined) const {
...
bool TransformedPrimitive::Intersect(const Ray<world_s> &r,
                                     Intersection *isect) const {
    Transform<world_s, world_s> w2p;
    WorldToPrimitive.Interpolate(r.time, &w2p);
    Ray<world_s> ray = w2p(r);
    ...
    if (!w2p.IsIdentity()) {
        ...
        Transform<world_s, world_s> PrimitiveToWorld = Inverse(w2p);
        isect->dg.p = PrimitiveToWorld(isect->dg.p);
        isect->dg.nn = PrimitiveToWorld(isect->dg.nn);
...
bool TransformedPrimitive::IntersectP(const Ray<world_s> &r) const {
...
BBox<world_s> GeometricPrimitive::WorldBound() const {
    return shape->WorldBound();
}


bool GeometricPrimitive::IntersectP(const Ray<world_s> &r) const {
...
bool GeometricPrimitive::Intersect(const Ray<world_s> &r,
                                   Intersection *isect) const {
